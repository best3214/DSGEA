...
class Evaluator():
    ...
    def csls_sim(self, embed1: np.ndarray, embed2: np.ndarray, k=10):
        ...
        # print("csls_sim-----sim_mat:", sim_mat[:100])
        # with open('sim_mat.txt', 'w', encoding='utf-8') as f:
        #     for row in sim_mat:
        ...
        # 目的：2*sim_mat - csls1 - csls2
        csls_sim_mat = self.compute_csls(sim_mat, csls1, csls2)
        # csls_sim_mat = np.where(csls_sim_mat < 0, 0, csls_sim_mat)
        # print("csls_sim_mat:", csls_sim_mat[:100])
    ...
    def dis_sim(self, embed1: np.ndarray, embed2: np.ndarray, k=10):
        embed1 = torch.tensor(embed1)
        embed2 = torch.tensor(embed2)
        t1 = time.time()
        # 计算相似度矩阵（n*m）
        S = torch.cdist(embed1, embed2, p=1)
        max_values = S.max(dim=1).values

        # 从每一行的每个元素中减去最大值
        S_centered = S - max_values.unsqueeze(1)
        sim_mat = F.softmax(S_centered, dim=1)
        sim_mat_inv = F.softmax(S_centered.t(), dim=1)
        t2 = time.time()
        print("计算相似度花费时间：",t2 - t1)

        return sim_mat, sim_mat_inv
