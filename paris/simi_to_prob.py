...
class SimiToProbModule:
    ...
    def train_model(self, simi_mtx, train_alignment, test_alignment):
        if self.inv:
            ...
            # print("simi_mtx.size(), simi_mtx.transpose(1,0).size():", simi_mtx.shape, simi_mtx.transpose(1, 0).shape)
    ...
    def save(self):
        ...
        if self.inv:
            torch.save(self.model.state_dict(), os.path.join(self.output_dir, "simi2prob_model_inv.ckpt"))
        else:
            torch.save(self.model.state_dict(), os.path.join(self.output_dir, "simi2prob_model.ckpt"))

    def load(self):
        if self.inv:
            self.model.load_state_dict(torch.load(os.path.join(self.output_dir, "simi2prob_model_inv.ckpt")))
        else:
            self.model.load_state_dict(torch.load(os.path.join(self.output_dir, "simi2prob_model.ckpt")))

    def prob_loss(self, logits, labels, k=5):
        probs = F.softmax(logits, dim=-1)

        neg_probs, _ = torch.topk(probs, dim=-1, k=k, largest=True, sorted=False)
        neg_probs = torch.mean(neg_probs, dim=-1, keepdim=False)

        # pos_probs = probs[:, labels].squeeze()
        pos_probs = torch.index_select(probs, 1, labels)

        loss = torch.sum(torch.abs(neg_probs - pos_probs))
        return loss
    ...
    def generate_features(self, simi_mtx: np.ndarray, alignment: np.ndarray = None):
        # print("simi_mtx.device", simi_mtx.device)
        # print("alignment.device", alignment.device)
