...
class Data():
    def __init__(self, kg1_triples, kg2_triples, alignment):
...
def apply_reasoning_torch(ent_arr: torch.Tensor, candidates_mtx: torch.Tensor, prob_mtx: torch.Tensor, data, device, inv=False):
    ent_arr = ent_arr.to(device)
    candidates_mtx = candidates_mtx.to(device)
    new_probs_list = []
    row_num, col_num = candidates_mtx.shape
    with torch.no_grad():
        # for row_idx in trange(row_num, desc="run reasoning with paris"):
        for row_idx in trange(row_num, desc="paris reasoning"):
            ...
            for col_idx in range(col_num):
                ent2 = candidates_mtx[row_idx][col_idx].cpu().item()
                if inv:
                    relnheads1 = data.tail_to_relnheads_map2[ent1]
                    relnheads2 = data.tail_to_relnheads_map1[ent2]
                    relnheads1_arr = torch.tensor(relnheads1, device=device)
                    relnheads2_arr = torch.tensor(relnheads2, device=device)
                    p = single_reasoning_torch(relnheads1_arr, relnheads2_arr, prob_mtx, data, device, inv)
                    probs.append(p)

                else:
                    relnheads1 = data.tail_to_relnheads_map1[ent1]
                    relnheads2 = data.tail_to_relnheads_map2[ent2]
                    relnheads1_arr = torch.tensor(relnheads1, device=device)
                    relnheads2_arr = torch.tensor(relnheads2, device=device)
                    p = single_reasoning_torch(relnheads1_arr, relnheads2_arr, prob_mtx, data, device)
                    probs.append(p)


            new_probs_list.append(probs)
    print("len(new_probs_list), len(new_probs_list[0]):", len(new_probs_list), len(new_probs_list[0]))
    new_probs_mtx = torch.tensor(new_probs_list, dtype=torch.float32, device=device)
    return new_probs_mtx

def apply_reasoning_torch1(neighbors1: np.array, neighbors2: np.array, other_entities1: torch.Tensor, other_entities2: torch.Tensor, prob_mtx: torch.Tensor, data, device, ratio=args.joint_distr_thr):
    # ent_arr = ent_arr.to(device)
    # candidates_mtx = candidates_mtx.to(device)
    row_num1, row_num2 = len(neighbors1), len(neighbors2)
    new_alignment_dict1 = dict()
    new_alignment_dict2 = dict()
    new_alignment = []
    ptrue = 0
    pfalse = 0
    print('ratio', ratio)
    ...
    for i in data.temp_set.cpu().numpy():
    # for i in train_alignment:
        new_alignment.append(i)
    # row_num, col_num = candidates_mtx.shape
    with torch.no_grad():
        # for row_idx in trange(row_num, desc="run reasoning with paris"):
        for row_idx in trange(row_num1, desc="paris reasoning"):
            row1 = np.intersect1d(neighbors1[row_idx], other_entities1.numpy())
            col_num1 = len(row1)
            row2 = np.intersect1d(neighbors2[row_idx], other_entities2.numpy())
            col_num2 = len(row2)
            new_probs_list = []
            if row1.size > 0 and row2.size > 0:
                for col_idx1 in range(col_num1):
                    ent1 = neighbors1[row_idx][col_idx1]
                    probs = []
                    for col_idx2 in range(col_num2):
                        ent2 = neighbors2[row_idx][col_idx2]
                    ...
                    new_probs_list.append(probs)
                new_probs_tensor = torch.tensor(new_probs_list, device=prob_mtx.device)
                new_probs_tensor_inv = new_probs_tensor.t()
                for num in range(col_num1):
                    probs = new_probs_tensor[num]
                    # print(probs)
                    prob, tmp_idx = torch.max(probs, dim=-1, keepdim=False)
                    prob = prob.cpu().item()
                    tmp_idx = tmp_idx.cpu().item()
                    probs_inv = new_probs_tensor_inv[tmp_idx]
                    ...
                    if num == tmp_idx_inv:
                        if prob > ratio and prob_inv > ratio:
                            if neighbors1[row_idx][num] not in new_alignment_dict1 and neighbors2[row_idx][tmp_idx] not in new_alignment_dict2:
                                new_alignment_dict1[neighbors1[row_idx][num]] = prob
                                new_alignment_dict2[neighbors2[row_idx][tmp_idx]] = prob_inv
                            elif neighbors1[row_idx][num] in new_alignment_dict1:
                                if new_alignment_dict1[neighbors1[row_idx][num]] <= prob:
                                    value_to_remove = neighbors1[row_idx][num]  # 要移除的值
                                    new_alignment = [row for row in new_alignment if row[0] != value_to_remove]
                                else:
                                    continue
                            elif neighbors2[row_idx][tmp_idx] in new_alignment_dict2:
                                if new_alignment_dict2[neighbors2[row_idx][tmp_idx]] <= prob_inv:
                                    value_to_remove = neighbors2[row_idx][tmp_idx]  # 要移除的值
                                    new_alignment = [row for row in new_alignment if row[1] != value_to_remove]
                                else:
                                    continue

                            # found = any(np.array_equal(np.array([neighbors1[num], neighbors2[tmp_idx]]),
                            #                            np.array(item)) for item in new_alignment)
                            # if not found:
                            new_alignment.append(np.array([neighbors1[row_idx][num], neighbors2[row_idx][tmp_idx]]))

        data.train_set = torch.tensor(new_alignment, device=data.train_set.device)
...
def single_reasoning_torch(relnheads1: torch.Tensor, relnheads2: torch.Tensor, prob_mtx: torch.Tensor, data, device, inv=False):
    rel1_arr = relnheads1[:, 0]
    head1_arr = relnheads1[:, 1]
    rel2_arr = relnheads2[:, 0]
    head2_arr = relnheads2[:, 1]
    repeat_times1 = head2_arr.shape[0] if isinstance(head2_arr.shape[0], int) else head2_arr.shape[0].item()
    repeat_times2 = head1_arr.shape[0] if isinstance(head1_arr.shape[0], int) else head1_arr.shape[0].item()

    if inv:
        equiv_prob = prob_mtx[head1_arr.to(prob_mtx.device)][:, head2_arr.to(prob_mtx.device)].to(device)
        sub_rel1_rel2 = data.sub_rel2_rel1_mtx[rel1_arr][:, rel2_arr]
        sub_rel2_rel1 = data.sub_rel1_rel2_mtx.transpose(dim0=0, dim1=1)[rel1_arr][:, rel2_arr]
        func1 = data.func_arr2[rel1_arr].unsqueeze(dim=1).repeat(repeats=(1, repeat_times1))
        func2 = data.func_arr1[rel2_arr].unsqueeze(dim=0).repeat(repeats=(repeat_times2, 1))
    else:
        # print("prob_mtx.device, data.sub_rel1_rel2_mtx.device, rel1_arr.device", prob_mtx.device, data.sub_rel1_rel2_mtx.device, rel1_arr.device)
        equiv_prob = prob_mtx[head1_arr.to(prob_mtx.device)][:, head2_arr.to(prob_mtx.device)].to(device)
        sub_rel1_rel2 = data.sub_rel1_rel2_mtx[rel1_arr][:, rel2_arr]
        sub_rel2_rel1 = data.sub_rel2_rel1_mtx.transpose(dim0=0, dim1=1)[rel1_arr][:, rel2_arr]
        func1 = data.func_arr1[rel1_arr].unsqueeze(dim=1).repeat(repeats=(1, repeat_times1))
        func2 = data.func_arr2[rel2_arr].unsqueeze(dim=0).repeat(repeats=(repeat_times2, 1))
